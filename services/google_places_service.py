...
- Uses Google Maps Places API to search for shops, cafes, etc.
- Returns results in the same place dict format used across the app.
"""
        ...
        try:
            ...
            if location:
                try:
                    ...
                    # If nearby search didn't find enough results, expand radius and retry once
                    if len(nearby_places) == 0:
                        try:
                            expanded_radius = max(15000, int(radius) * 3)
                            resp_nearby2 = self.client.places_nearby(location=(location['lat'], location['lng']),
                                                                     radius=expanded_radius,
                                                                     keyword=query)
                            nearby2 = resp_nearby2.get('results', [])
                            logger.info(f"Expanded nearby search (radius {expanded_radius}m) found {len(nearby2)} results for '{query}'")
                            all_places.extend(nearby2)
                        except Exception as e:
                            logger.warning(f"Expanded nearby search failed: {e}")
            ...
            if location and len(all_places) > 0:
                ...
                # Filter out results that are too far (be less aggressive to avoid zero results)
                # UAE can be spread out; allow up to 80km. Elsewhere allow up to 60km.
                max_distance_km = 80 if self._is_in_uae_region(location) else 60
                filtered_places = []
                ...
                if filtered_places:
                    ...
                    # If filtering removed everything, keep the closest 10 without strict distance cap
                    logger.info("Distance filtering removed all results; returning closest available places without cap")
            ...
            if out:
                ...
                for i, place in enumerate(out[:3], 1):  # Log first 3 results
